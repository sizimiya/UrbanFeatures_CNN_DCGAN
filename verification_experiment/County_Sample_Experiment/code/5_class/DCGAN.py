...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.linear1 = nn.Linear(100, 256 * 2 * 2)
        self.bn1 = nn.BatchNorm1d(256 * 2 * 2)

        self.deconv1 = nn.ConvTranspose2d(256, 128,
                                          kernel_size=(3, 3),
                                          stride=1,
                                          padding=1)  # 生成（128，2，2）的二维图像
        self.bn3 = nn.BatchNorm2d(128)
        self.deconv2 = nn.ConvTranspose2d(128, 64,
                                          kernel_size=(4, 4),
                                          stride=2,
                                          padding=1)  # 生成（64，4，4）的二维图像
        self.bn4 = nn.BatchNorm2d(64)
        self.deconv3 = nn.ConvTranspose2d(64, 1,
                                          kernel_size=(3, 3),
                                          stride=2,
                                          padding=1)  # 生成（1,7,7）的二维图像
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 64, kernel_size=3, stride=2)  # 64*3*3
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, stride=2)  # 128*1*1
...
for i in range(epoch):
    ...
    for step, x in enumerate(dl):
        ...
        with torch.no_grad():
            ...
            data_singel = fake.reshape(1, 1 * 7 * 7)
