...
class CsvDataset(Dataset):
    def __init__(self):
        ...
        assert feature_df_[feature_df_.columns[0]].tolist() == label_df_[label_df_.columns[0]].tolist(), \
            'feature name does not match label name'
        feature_df_.set_index(feature_df_.columns[0], inplace=True)
        self.feature = [feature_df_.loc[i] for i in feature_df_.index[0:]]
...
testloader = DataLoader(dataset=testset, batch_size=4528, shuffle=True)
...
D = discriminator()
checkpoint = torch.load('./model/Discriminator_cuda_70.pkl', map_location='cpu')
