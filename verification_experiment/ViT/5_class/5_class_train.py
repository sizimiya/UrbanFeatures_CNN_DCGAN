...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_c=1, num_classes=5,                              #depth是encoder block重复的次数  num_heads是head的个数  mlp_ratio是mlp中节点扩充的倍数
...
def vit8(num_classes: int = 5, has_logits: bool = True):
...
for i in range(epoch):
 ...
 # 类标签
        ...
        # 真图片的损失
        real_cls_out = vit(img)  # 真图片送入判别器D  得到真假输出 100*1 和分类输出100*15(10)
    ...
    if (i % 10 == 0) and (i != 0):
        print(i)
        torch.save(vit.state_dict(), r'./ACGAN_model_save/Discriminator_cuda_%d.pkl' % i)
